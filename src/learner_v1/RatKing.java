...
// initialize king ***********************
    public static void initialize_king() throws GameActionException {
        ...
        // econ5-style king location tracking: indices 0-9 = 5 king location pairs
        write_loc(0, rc.getLocation());

        // counter / king count moved to indices 61/62 to avoid conflict
        king_counter=Math.max(rc.readSharedArray(61)-1,0);
        rc.writeSharedArray(61,king_counter);
        kings=rc.readSharedArray(62)+1;
        rc.writeSharedArray(62,kings);
        king_num = kings;
...
// compute number of kings and record in shared array
    public static void num_kings() throws GameActionException{
        if (king_counter>=1023){
            king_counter%=1024;
            if(king_counter==rc.readSharedArray(61)){
                kings=rc.readSharedArray(62);
                ...
                kings-=1;
                rc.writeSharedArray(62,kings);
                rc.writeSharedArray(61,king_counter+1);
                king_counter+=kings;
                king_counter%=1024;
            }
        }
...
//////////////////// end M variables/functions ///////////////////////////
    ...
    */
    ...
    static void writes() throws GameActionException {
        for (int i = 0; i < 5; i++) {
            MapLocation loc = read_loc(2 * i);
            if (loc.x == 0 && loc.y == 0) {
