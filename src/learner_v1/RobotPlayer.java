        ...
        while (true) {
            ...
            try {
                ...
                if (enemies.length == 0) {
                    ...
                    rc.setIndicatorString("NN m" + bestMoved + " f" + bestFacing);
                    MapLocation here = rc.getLocation();
                    Direction preferredDir = bestMoved > 0 ? allDirections[bestMoved] : allDirections[bestFacing];
                    if (preferredDir != Direction.CENTER) {
                        MapLocation there = here.add(preferredDir);
                        int mapW = rc.getMapWidth(), mapH = rc.getMapHeight();
                        there = new MapLocation(Math.max(0, Math.min(mapW - 1, there.x)), Math.max(0, Math.min(mapH - 1, there.y)));
                        rc.setIndicatorLine(here, there, 255, 200, 0);
                    }
